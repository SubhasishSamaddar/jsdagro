...

{
	...
	public function __construct(Request $request){
		...
		{
			Session::put('swadesh_hut_name', 'uttarpara');
			Session::put('swadesh_pin_code', '712232');
			Session::put('swadesh_hut_id', '3');
		}
	}
	...
	public function details(Request $request, $name_url){
		...
		if(Session::has('swadesh_hut_id')):
			...
			$details = DB::table('products as A')
		 	...
		 	$get_products_weight = DB::table('products')->select('sku','weight_per_pkt','weight_unit','name_url')
			->where('prod_name',$details->prod_name)
			...
			$sku_count = 0;
			$similar_count = 0;
			foreach($get_products_weight as $wt_dtls){
				$set_sku_array[$sku_count] = $wt_dtls->sku;
				$sku_count++;
			}

			if(in_array($details->similar_product,$set_sku_array)){
				$similar_count++;
			}


			if($similar_count>0){
				$link_similar_products = DB::table('products')->select('prod_name','sku','weight_per_pkt','weight_unit','name_url', 'swadesh_hut_id')
				->where('prod_name',$details->prod_name)
				->where('swadesh_hut_id', $swadesh_hut_id)
				->orderBy('weight_per_pkt','ASC')->get();
			}
			else {
				$link_similar_products = array();
			}
			...
			/*$link_similar_products = DB::table('products')->select('id','prod_name','price','weight_per_pkt', 'weight_unit','discount','wholesale_price', 'swadesh_hut_id', 'name_url')
			->where('similar_product', $details->sku)
			->where('swadesh_hut_id', $swadesh_hut_id)
			->whereNotNull('similar_product')
			->orderBy('weight_per_pkt','ASC')
			->get();*/
	...
	public function productCategoryListing($name_url){
		...
		if($getcategorydetails->parent_id==0)
		{
			...
			if(Session::has('swadesh_hut_id')):
				...
				->where(function ($query){
					$query->whereRaw('similar_product = sku')->orWhereNull('similar_product');
				})
				//->groupBy('prod_name')
		...
		$categorydetails = DB::table('categories')->select('name','category_banner_images','parent_id')->where('id', $cat_id)->first();
	...
	public function get_product_price_by_id(Request $request)
	{
		...
		$get_price = DB::table('products')->select('id','price','max_price','weight_per_pkt','weight_unit','discount','available_qty','ordered_qty')->where('id',$product_id)->first();

		if( count(Cart::getContent())>0 ):
			foreach( Cart::getContent() as $cartData ):
				$productIdName=explode('%',$cartData->name);
				if( $productIdName[0]==$get_price->id ):
					if( $cartData->attributes['product_availability']>$cartData->quantity):
						$buttonDisable=0; 
					else:
						$buttonDisable=1;
					endif;
				elseif( ($get_price->available_qty-$get_price->ordered_qty)>0 ):
					$buttonDisable=0;
				else:
					$buttonDisable=1;
				endif;
		...
		//echo $buttonDisable;exit;

		return response()->json(['price'=>$get_price->price,'max_price'=>$get_price->max_price,'weight_per_pkt'=>$get_price->weight_per_pkt,'weight_unit'=>$get_price->weight_unit,'discount'=>$get_price->discount,'buttonDisable'=>$buttonDisable]);
	...
	public function getExactProductByBarcode(Request $request) 
	{
		...
		$product_html = '';
		$get_exact_products_by_barcode = DB::table('products')->where('barcode', $barcode_text)->where('swadesh_hut_id',Session::get('pos_swadesh_hut'))->first();
			...
			$product_html.='<div class="w-140px w-xl-180px w-xxl-210px mx-2" onclick="setSessionIdArray(\''.$get_exact_products_by_barcode->id.'\')">
				<div class="card bg-white c-pointer product-card hov-container">
					...
					<div class="card-body p-2 p-xl-3">
						...
						<div class="price">';
							if(number_format($get_exact_products_by_barcode->max_price,2)!=number_format($pro_price,2)) {
								$product_html.='<del class="mr-2 ml-0">₹'.number_format($get_exact_products_by_barcode->max_price,2).'</del>';
							}
	...
	public function generateCartData(Request $request)
	{
		...
		<div class="d-flex justify-content-between fw-600 fs-18 border-top pt-2">
			<span>TOTAL</span>
			<input type="text" name="form_discount_amount" id="form_discount_amount" onkeyup="set_discounted_price()" placeholder="Enter Discount Amount" style="width:200px;margin-bottom:10px;margin-left:10px;">
