...
class CheckoutController extends Controller
{
	...
	public function shipping( Request $request ){
		if( !Cart::getContent()->isEmpty() ):
			...
			return view('front.checkout.shipping',compact('data', 'billingDetails', 'countryDetails', 'stateDetails', 'categories', 'siteBarProducts', 'parentCategory'));
		else:
			//echo 'Here';die;
	...
	/*public function paymentWithPayu($postRequest)
	{
		$productIdName='test';

		$attributes = [
			'txnid' => strtoupper(str_random(8)), # Transaction ID.
			'amount' => $postRequest['total_amount'], # Amount to be charged.
			'productinfo' => $productIdName,
			'firstname' => $postRequest['billing_name'], # Payee Name.
			'email' => $postRequest['billing_email'], # Payee Email Address.
			'phone' => $postRequest['billing_phone'], # Payee Phone Number.
			'address1' => $postRequest['shipping_address'],
			'address2' => $postRequest['billing_name'],
			'city' => $postRequest['billing_name'],
			'state' => $postRequest['billing_name'],
		];
	...
	public function status(Request $request)
	{
		...
		if($payment->isCaptured())
		{
			...
			$payment_attribute_data = json_decode($payment->data);
			$transaction_array = array(
				'txn_id'=>$payment_attribute_data->txnid,
				'user_id'=>Auth::user()->id,
				'amount'=>$payment_attribute_data->amount,
				'product_info'=>$payment_attribute_data->productinfo,
				'user_email'=>$payment_attribute_data->email,
				'user_phone'=>$payment_attribute_data->phone
			);
			...
			$parentCategory = Category::Where('parent_id','0')->Where('status','Active')->limit(12)->get();
			$get_order_details = DB::table('order_master')->where('id',$data['order_id'])->first();
			return view('front.checkout.confirm',compact('parentCategory','data','get_order_details'));
		}
	}
