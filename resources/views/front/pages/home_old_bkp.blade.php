...
<div class="featured_products">
	...
	@if( !$showinHomePageCategory->isEmpty() )
			...
			@foreach( $showinHomePageCategory as $shData )
				...
				$catProduct=Helper::get_product_by_categorys($shData->id, $swadesh_hut_id);
					...
					<li>
						...
						?>
							...
							<div class="thumbnail">
								...
								<a  href="{{ route('products',$cpdata->id) }}">
							...
							<div class="caption">
								...
								<h5><a href="{{ route('products',$cpdata->id) }}">{{ $cpdata->prod_name }}</a></h5>
								...
								<h3 id="cunit_container_{{ $cpdata->id }}">{{ $cpdata->weight_per_pkt }} {{ $cpdata->weight_unit }}</h3>
								...
								<select class="form-control" id="product_weight_price" onchange="get_cproduct_price(this,{{ $cpdata->id }})" style="width:180px;margin-left:2px;">
...

									{
										foreach($optionProduct as $option)
										{
											...
											//$final_price = ((100-$option->discount)*$option->price/100);
