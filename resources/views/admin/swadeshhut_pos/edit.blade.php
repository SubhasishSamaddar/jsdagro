...
@section('content')
	...
	<div class="card">
		<div class="card-header">
			<div class="pull-left">
	            <h4>Edit Store Pos</h4>
	        </div>
        </div>

		<div class="card-body">
        <form method="post" action="{{ route('swadesh_hut_pos.update', $details->id) }}" enctype="multipart/form-data">
        @csrf
        @method('PATCH')
        <div class="row">
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="name" id="name"  value="{{$details->name}}" required/>
                </div>
                
				<div class="form-group">
                    <label for="prod_name">User Name</label>
                    <input type="text" class="form-control" name="userName" id="userName"  value="{{$details->userName}}" required/>
                </div>
				<div class="form-group">
                    <label for="prod_name">Password</label>
                    <input type="text" class="form-control" name="password" id="password"  value="{{$details->password}}" required/>
                </div>
                ...
                <?php if(Auth::user()->user_type !='Swadesh_Hut') { ?>
                 <div class="form-group">
					...
					<select name="swadeshhut_id" id="swadeshhut_id" class="form-control" required >
						...
						<option value="{{ $value->id}}" {{ ($value->id==$details->swadeshhut_id)?'selected':''}}>{{ $value->location_name}}</option>
                        @endforeach
                    </select> 
				</div>
				<?php } ?>
