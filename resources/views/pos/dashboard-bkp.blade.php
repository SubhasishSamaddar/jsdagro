...
function filterProducts(){
	...
	$.ajax({      
		type: "POST",
		url: "{{ url('filter-products-by-barcode') }}",   
    ...
    $.ajax({      
		...
		function(data){
            ...
            else 
            {
                ...
                if(product_count==0){
                    $("#productcartlist").append(data.list_html);
                    $("#cart_price_details").html('<div class="d-flex justify-content-between fw-600 fs-18 border-top pt-2"><span>TOTAL</span><span>₹<span id="total_cart_amount">'+(totalPrice.toFixed(2))+'</span></span></div><input type="hidden" id="form_total_cart_amount" name="form_total_cart_amount" value="'+(totalPrice.toFixed(2))+'">');
                    var curr_price = 0;
                    $('input[name="form_product_price[]"]').each( function() {
